...
import { articles as articleMap } from "@/lib/blog-articles";
...
const curated = [
  {
    ...
    excerpt:
      "Snow removal is the only trade where the size of the job is decided by the sky after you've already agreed to the price. The invoice you send in January was effectively written by the contract you signed in September — which is why the contractors who lose money in this business rarely lose it plowing. They lose it on a flat seasonal rate in a heavy year, on a per-push clause vague enough that a client can call a 30-hour blizzard one push, or on a season's worth of salt priced before the shortage. This guide covers how to invoice for snow removal and plowing properly: choosing between per-push, per-inch tiered, hourly, seasonal flat-rate, and capped-event billing and what each does to you in a light winter versus a brutal one; writing the trigger depth, measurement source, and event-reset clause that decide how many pushes one storm is; billing salt and de-icing on their own lines with a material escalator; the time-stamped service documentation that is simultaneously your fastest path to approval and your defense when a slip-and-fall claim arrives eight months later; levelized monthly billing and how to answer the warm-December phone call; commercial COIs, PO numbers, per-site subtotals, and net terms; the accessorials most contractors perform and never bill, including pre-treatment on a storm that missed; residential prepay and card-on-file; and what suspending service for nonpayment actually exposes you to.",
  },
  {
    slug: "how-to-invoice-for-tutoring",
    title: "How to Invoice for Tutoring & Test Prep (Packages, No-Shows & School District POs, 2026)",
    excerpt:
      "Tutoring is the trade where the person sitting in the session is almost never the person paying for it, and where the money usually arrives before the work exists. Both facts change how you invoice. This guide covers billing for tutoring and test prep properly: choosing between per-session billing, prepaid packages, monthly tuition, and district contracts; why a prepaid package is a liability on your books rather than income you've earned, and how to show sessions used and sessions remaining so a parent can see it; the cancellation and no-show policy that decides whether your calendar is worth anything; invoicing the actual payer when that's a parent, a separated household, an agency, or a school district; getting through a district's purchase order, W-9, vendor registration, and background-check requirements before you can be paid at all; whether tutoring is exempt from sales tax in your state and what flips it; how agency and platform work shows up on your 1099; and how to smooth the summer trough that every tutoring business runs into.",
  },
  {
...
// Any article the curated list above forgets still gets a card, newest first,
// falling back to its own title and description. See the note on `curated`.
const uncurated = Object.keys(articleMap)
  .filter((slug) => !curated.some((a) => a.slug === slug))
  .sort((a, b) => (publishedAt[b] ?? "").localeCompare(publishedAt[a] ?? ""))
  .map((slug) => ({
    slug,
    title: articleMap[slug].title,
    excerpt: articleMap[slug].description,
  }));

const articles = [...uncurated, ...curated];
